...
import { DEPT_API, EMP_API } from "../config.js"; // backend API urls
...
const ManageDepartment = () => {
  ...
  // FETCH EMPLOYEES FOR HEAD SELECT
useQuery({
  queryKey: ["employees"],
  queryFn: async () => {
    const res = await axiosAuth.get(`${EMP_API}/get-all`);
  ...
  const createMutation = useMutation({
    mutationFn: (body) => axiosAuth.post(`${DEPT_API}/create-department`, body),
    onSuccess: () => {
  ...
  const updateMutation = useMutation({
    mutationFn: ({ id, body }) =>
      axiosAuth.put(`${DEPT_API}/update-department/${id}`, body),
    onSuccess: () => {
  ...
  const deleteMutation = useMutation({
    mutationFn: (id) => axiosAuth.delete(`${DEPT_API}/delete-department/${id}`),
    onSuccess: () => {
            ...
            <tbody>
              {departments.map((dept, index) => (
                <tr key={dept._id} className="border-b hover:bg-gray-50">
                  ...
                  <td className="p-3">{dept.head?.name || "N/A"}</td>
  ...
  loadOptions={async (inputValue) => {
    ...
    try {
      ...
      // Map data to format react-select needs
